...
Permitir que un administrativo modifique los datos de una sanción disciplinaria existente, como el motivo, el período de vigencia o el tipo de suspensión, en caso de error de carga o cambio en la decisión disciplinaria.
...
- El campo `member_id` no puede ser modificado para mantener la integridad histórica.
...
4. Crear la ruta `PUT /api/v1/disciplines/:id` en `DisciplineController` y enlazarla a la app de Fastify.
