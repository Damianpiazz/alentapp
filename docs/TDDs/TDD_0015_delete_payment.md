...
*   **Endpoint**: `PATCH /api/v1/payments/:id/cancelado`
*   **Request Body**: `None`

### Componentes de Arquitectura Hexagonal

1.  **Puerto**: PaymentRepository (Por una cuestión semántica, implementará el método delete(id)).
2.  **Servicio de Dominio**: PaymentValidator (Encargado de verificar que el pago no esté en estado "Pagado" previamente para evitar conflictos).
3.  **Caso de Uso**: DeletePaymentUseCase (Orquesta la validación utilizando el PaymentValidator y llama al repositorio).
4.  **Adaptador de Salida** : PostgresPaymentRepository (El método delete del repositorio ejecutará en realidad un update de Prisma para cambiar el status a "Cancelado").
...
| El pago ya está `Pagado`      | Mensaje: "No se puede anular un pago ya procesado"             | 409 Conflict              |
