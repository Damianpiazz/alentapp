...
4. **Adaptador de Entrada**: `LockerController` (Ruta HTTP que recibe el `id` y devuelve status `204`).
...
1. Ampliar `LockerRepository` y `PostgresLockerRepository` con el método `delete` (aunque no se elimine nada)
2. Crear la lógica de negocio en `ReleaseLockerUseCase`.
3. Crear el endpoint `DELETE /api/v1/lockers/:id` en `LockerController`.
4. Actualizar el estado del locker a `Disponible` y eliminar la relación con el socio.
5. Conectar la acción de liberación desde el frontend agregando confirmación previa.
