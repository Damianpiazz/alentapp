...
test.beforeAll(async () => {
    const client = new pg.Client({
        connectionString:
            'postgresql://admin:password123@localhost:5433/alentapp_test_db',
    });
...
test.describe('Members Full-Stack E2E', () => {
    test('debe mostrar el estado vacío cuando no hay miembros en la DB', async ({
        page,
    }) => {
    ...
    test('debe crear un miembro real y mostrarlo en la tabla', async ({
        page,
    }) => {
        await page.goto('/members');
    ...
    test('debe editar el miembro creado y ver el cambio en la tabla', async ({
        page,
    }) => {
        await page.goto('/members');
        ...
        await page
            .getByPlaceholder('Ej. Juan Pérez')
            .fill('Test E2E Fullstack Editado');
        ...
        await expect(page.getByText('Test E2E Fullstack Editado')).toBeVisible({
            timeout: 10000,
        });
        await expect(
            page.getByText('Test E2E Fullstack', { exact: true }),
        ).toBeHidden();
    });

    test('debe eliminar el miembro y mostrar el estado vacío', async ({
        page,
    }) => {
        await page.goto('/members');
        ...
        // Clic en borrar
        await page
            .getByRole('button', { name: /Eliminar miembro/i })
            .first()
            .click();
